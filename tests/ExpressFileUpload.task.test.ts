...
describe(ExpressFileUploadTask, (): void => {
  it('behaves as expected', async (): Promise<void> => {
    await jestCore.execTask('express-fileupload-task', {
      directive: 'init',
    ...
    expect(populateTemplates).toHaveBeenCalledWith(expect.stringMatching(/universal-core-express-controllers-fileupload\/src\/template/), './src', { override: true })
  })

  it('throws an error if directive is not recognized', async (): Promise<void> => {
    await expect(
      jestCore.execTask('express-fileupload-task', {
        directive: 'nop',
